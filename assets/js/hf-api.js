...
function hfFetch() {
  ...
  fetch(base + "api/models/" + modelId, opts)
    ...
    .then(function (result) {
      ...
      // Build the canonical imported-model object.
      // baseModel / baseModelRelation carry HF lineage metadata so the
      // variant grid can display the correct type label (FINE-TUNE, ADAPTER,
      // QUANTIZED, MERGE …) instead of always showing “BASE MODEL”.
      gImp = {
        tType: tType,
        tBpb: tBpb,
        ...
        // HF lineage fields (from model card YAML)
        baseModel: Array.isArray(card.base_model)
          ? card.base_model.join(", ")
          : card.base_model || null,
        baseModelRelation: card.base_model_relation || null,
      };
